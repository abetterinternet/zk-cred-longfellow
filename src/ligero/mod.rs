...
/// Gather the elements at the provided indices from source, in the order of the indices. As
/// specified in [2.1][1].
///
...
pub fn gather_iter<FE: LagrangePolynomialFieldElement>(
    ...
    indices.iter().map(|index| source[*index])
}

pub fn gather<FE: LagrangePolynomialFieldElement>(source: &[FE], indices: &[usize]) -> Vec<FE> {
    gather_iter(source, indices).collect()
}
