            ...
            circuit.layers.iter().zip(proof.layers.iter()).enumerate()
        {
            ...
            for (round, polynomial_pair) in proof_layer.polynomials.iter().enumerate() {
                for (hand, polynomial) in polynomial_pair.iter().enumerate() {
                    ...
                    // p0, known part:
                    claim_known +=
                        polynomial.p0 * FE::lagrange_basis_polynomial_i(FE::ZERO, challenge[0]);
                    ...
                    constraints
                        ...
                        .push(LinearConstraintLhsTerm {
                            ...
                            constant_factor: FE::lagrange_basis_polynomial_i(
                                FE::ZERO,
                                challenge[0],
                            ) - FE::lagrange_basis_polynomial_i(
                                FE::ONE,
                                challenge[0],
                            ),
                        });
                    ...
                    // p2, known part:
                    claim_known += polynomial.p2
                        * FE::lagrange_basis_polynomial_i(FE::SUMCHECK_P2, challenge[0]);
