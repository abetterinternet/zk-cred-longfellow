...
impl LigeroCommitment {
    ...
    {
        // Rows for the witnesses -- does this include the quad constraint witnesses?
        ...
        // Fill the linear test rot ("IDOT"): random field elements where elements [nreq..nreq+wr)
        ...
        let mut z: Vec<_> = std::iter::from_fn(|| {
            let element = if index == parameters.nreq {
                ...
                let element = element_generator.next();
