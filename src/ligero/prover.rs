...
impl<'a> LigeroProver<'a> {
    ...
    pub fn prove<FE: CodecFieldElement + LagrangePolynomialFieldElement>(
        ...
        Ok(LigeroProof {
            ...
            tableau_columns: requested_tableau_columns,
            inclusion_proof,
            merkle_tree_nonces,
        })
    }
...
impl<FE: CodecFieldElement> LigeroProof<FE> {
    ...
    pub fn quadratic_proof(&self, tableau_layout: &TableauLayout) -> Vec<FE> {
        ...
        proof.resize(
            proof.len() + tableau_layout.block_size() - tableau_layout.num_requested_columns(),
            FE::ZERO,
        );
        proof.extend(&self.quadratic_proof.1);
        assert_eq!(proof.len(), tableau_layout.num_columns());
